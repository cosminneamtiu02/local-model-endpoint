"""Lifespan-managed resource construction (split from router_registry).

main.py imports ``lifespan_resources`` here. The registry split keeps
``router_registry.py`` a single-purpose registry (router mounting only)
mirroring the single-purpose ``exception_handler_registry.py`` — the
``*_registry.py`` filename advertises a registry, and a future reviewer
searching for the lifespan resource factory now finds it under the
matching ``lifespan_resources.py`` path.
"""
