...
7. **Test rewrites:** `tests/integration/conftest.py` (no DB, ASGI transport client), `tests/integration/api/test_health.py` (just /health + request-id; later renamed to `test_health_router.py` to mirror its source `app/api/health_router.py`), `tests/contract/test_schemathesis.py` (drop DB env, drop widget endpoint assertions — file later replaced by `test_openapi_shape.py` (later renamed to `test_openapi_document_validity.py`) + `test_problem_details_contract.py` (now `test_problem_details_shape.py`, with route-level concerns split out into `test_health_route_publication.py`)), `tests/unit/exceptions/test_domain_errors.py` (substitute RateLimitedError for widget), `tests/unit/exceptions/test_error_handler.py` (substitute RateLimitedError for widget — file later moved to `tests/unit/api/test_exception_handlers.py`, then renamed to `tests/unit/api/test_exception_handler_registry.py`).
