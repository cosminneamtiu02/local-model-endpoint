...
7. **Test rewrites:** `tests/integration/conftest.py` (no DB, ASGI transport client), `tests/integration/api/test_health.py` (just /health + request-id), `tests/contract/test_schemathesis.py` (drop DB env, drop widget endpoint assertions — file later replaced by `test_openapi_shape.py` + `test_problem_details_contract.py` (now `test_problem_details_shape.py`)), `tests/unit/exceptions/test_domain_errors.py` (substitute RateLimitedError for widget), `tests/unit/exceptions/test_error_handler.py` (substitute RateLimitedError for widget — file later moved to `tests/unit/api/test_exception_handlers.py`, then renamed in round 18 to `tests/unit/api/test_exception_handler_registry.py`).
