...
def test_codegen_produces_valid_python(sample_errors_path: Path, output_dir: Path):
    ...
    content = (output_dir / "example_not_found_error.py").read_text(encoding="utf-8")
    ...
    internal_content = (output_dir / "internal_error.py").read_text(encoding="utf-8")
    ...
    registry_content = (output_dir / "_registry.py").read_text(encoding="utf-8")
    ...
    params_content = (output_dir / "example_name_too_long_params.py").read_text(encoding="utf-8")
...
def test_codegen_rejects_duplicate_codes(tmp_path: Path):
...
def test_codegen_rejects_invalid_http_status(tmp_path: Path):
    """Codegen should reject error codes with non-error HTTP status."""
    path = tmp_path / "errors.yaml"
    path.write_text(INVALID_STATUS_YAML, encoding="utf-8")
...
def test_codegen_rejects_invalid_param_type(tmp_path: Path):
    """Codegen should reject params with unsupported types."""
    path = tmp_path / "errors.yaml"
    path.write_text(INVALID_PARAM_TYPE_YAML, encoding="utf-8")
...
def test_codegen_against_production_yaml_matches_committed_tree(tmp_path: Path):
    """Regenerate from production errors.yaml; verify output matches committed _generated/."""
    from scripts.generate import generate_python

    # packages/error-contracts/tests/test_generate.py -> repo root is parents[3]
    repo_root = Path(__file__).resolve().parents[3]
    prod_yaml = repo_root / "packages" / "error-contracts" / "errors.yaml"
    committed_dir = repo_root / "apps" / "backend" / "app" / "exceptions" / "_generated"

    regen_dir = tmp_path / "_generated"
    generate_python(prod_yaml, regen_dir)

    # Compare every regenerated file to the committed one
    committed_files = {p.name for p in committed_dir.glob("*.py")}
    regenerated_files = {p.name for p in regen_dir.glob("*.py")}
    assert committed_files == regenerated_files, (
        f"File set drift: {committed_files ^ regenerated_files}"
    )

    for name in committed_files:
        committed_text = (committed_dir / name).read_text(encoding="utf-8")
        regenerated_text = (regen_dir / name).read_text(encoding="utf-8")
        assert committed_text == regenerated_text, f"Content drift in {name}"


def test_codegen_removes_orphan_files(tmp_path: Path):
    """Pre-existing .py files in output_dir should be removed before regeneration."""
    from scripts.generate import generate_python

    out = tmp_path / "_generated"
    out.mkdir()
    stale = out / "dropped_error.py"
    stale.write_text("# orphan from a deleted YAML entry\n", encoding="utf-8")

    yaml_path = tmp_path / "errors.yaml"
    yaml_path.write_text(SAMPLE_YAML, encoding="utf-8")
    generate_python(yaml_path, out)

    assert not stale.exists(), "Orphan file was not cleaned up"


def test_load_and_validate_rejects_duplicate_keys(tmp_path: Path):
    """SafeLoader override must catch duplicate mapping keys."""
    from scripts.generate import load_and_validate
...
def test_codegen_preserves_unicode_in_descriptions(tmp_path: Path):
    """UTF-8 round-trip: non-ASCII descriptions survive load + write."""
    from scripts.generate import generate_python
...
errors:
  UNICODE_TEST:
    http_status: 418
    description: "测试 — emoji 🚫 and en-dash"
    params: {}
"""
    path = tmp_path / "errors.yaml"
    path.write_text(utf_yaml, encoding="utf-8")
    out = tmp_path / "_generated"
    generate_python(path, out)
    text = (out / "unicode_test_error.py").read_text(encoding="utf-8")
    assert "测试" in text
    assert "🚫" in text


def test_load_and_validate_rejects_non_dict_params(tmp_path: Path):
    """Codegen must reject `params` whose value is not a mapping."""
    from scripts.generate import load_and_validate
